"""Copy flat image/label pairs into reproducible dataset splits."""
import argparse
import math
import random
import shutil
...
def main():
    ...
    try:
        ...
        if not images and not labels:
            ...
            return 0
        # Refuse populated destinations to prevent overwrites and repeated-run leakage.
        for kind in ('images', 'labels'):
            for split in SPLITS:
        ...
        # Largest-remainder rounding allocates every pair deterministically.
            ...
            for split, size in zip(SPLITS, sizes):
                ...
                for image in images[offset:offset + size]:
                    for kind, source in (('images', image), ('labels', label_map[image.stem])):
                        ...
                        with target.open('xb') as output:  # Never overwrite.
