"""Display and save detections from a trained VisionGuardAI model."""
import argparse
import os
import math
import time
...
def resolve_source(value):
    """Keep network URLs intact instead of interpreting them as Windows paths."""
    value = value.strip()
    if not value:
...
def main():
    ...
    parser.add_argument('--volume', type=float, default=.15, help='Sound amplitude 0 to 0.5; speaker volume also applies.')
    ...
    try:
        ...
        # Stream results instead of keeping a whole video in memory.
